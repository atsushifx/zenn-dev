...
systemd の有効化やホスト名固定、PATH の調整などを、たった 3 ステップで反映できます。
...
この記事では、WSL の起動動作を制御する設定ファイルである `/etc/wsl.conf` を用いて、以下のようなカスタマイズを行なう方法を紹介します。
...
`wsl.conf` は `.ini` 形式に準拠したシンプルな構成ファイルです。セクションごとに `[section]` ラベルを記述し、キーと値を `=` で結んで設定内容を定義します。
...
3. 自分の環境に合わせて、以下の項目を変更します。
   ...
   - `hostname`: `develop`など自分の好きな名前をつける
   - `default`:   起動時にログインするユーザーアカウント
                  指定しない場合は、**初回インストール時に設定した「既定のユーザー」**となる。
...
インストール中中: Linux 用 Windows サブシステム 2.4.13
[========================= 43.8%                           ]
```
...
この挙動により、WSL 内で Windows アプリ (例: `notepad.exe`, `explorer.exe`) を直接実行できます。

次のようなケースでは無効化 (`false`) することが推奨されます:
