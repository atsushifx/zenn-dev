...
WSL を使うことで、開発者は Linux上のツールやコマンドを使用でき、Web開発やバックエンド開発がスムーズに行なえます。
...
`/etc/wsl.conf`の設定を元ディストリビューションと開発ディストリビューションとで同じにすることで、設定の一貫性を保つことができます。
...
1. `/etc/wsl.conf`ファイルを作成する
   下記の内容で、`/etc/wsl.conf`を作成します。`#`以降はコメントです。
   ...
   **注** `<my_custom_hostname>`の部分を開発ディストリビューションのホスト名に、
