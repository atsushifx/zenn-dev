...
- **systemd の有効化**
  ...
  WSL 上で `Docker` をネイティブに実行させるために、`systemd` を有効化しておく必要があります。
...
*図2:  PowerShell を使用して `wsl.conf` を反映するフロー*
...
:::message alert
`systemd=true` の設定は WSL が正しく対応していないと反映されません。以下のチェック項目を順に確認してみましょう。
:::
