...
## 3. なぜ「CI/CD基盤の共通化」に向いているのか

Reusable Workflows が、なぜ「CI/CD 基板の共通化」に向いているかを、複数リポジトリ運用での問題点や共通化すべき責務などから明確化します。
...
これらは個々の workflow を見れば小さな差異ですが、リポジトリ数が増えるにつれて**運用リスクと保守コストが指数的に増大**します。
特にセキュリティや品質に関する判断が分散すると、「どれが正しい状態なのか」を把握すること自体が難しくなります。
...
結果として、**「CI/CD の正しさを決める責務」と「プロジェクトの事情を反映する責務」**が明確に分離されます。
...
「基盤」とは、reusable workflow が CI/CD の責務を引き受けると言うことであり、GitHub Actions の構造によって、このことを自然な形で実現します。
