...
次は、ダウンロードした`ISO イメージ`を使ってインストールメディアを作成します。

### 1.2 `winget`を使用した`Rufus`のインストール

インストールメディアの作成に、`Rufus`を使用します。
`Rufus`をインストールする手順は以下の通りです:

1. ターミナル (管理者) を起動
2. `winget` の確認
3. `winget`を使用した`Rufus`のインストール
4. 手動インストール (`winget`が利用できない場合)
...
3. **`Rufus`のインストール**:
   ...
   ```powershell
   コマンド ライン エイリアスが追加されました: "rufus"
   インストールが完了しました
   ```

   **エラーが表示された場合**:
   - ネットワーク接続を確認し、再度コマンドを実行する
   - **`winget`が使用できない** を参考に、直接`Rufus`をインストールする
...
4. **起動画面が表示されない場合**:
   ...
   くわしいことは、[トラブルシューティング](#22-トラブルシューティング)を参照してください。
...
- USBポートの問題:
　サーバーの USBポートには、USBメモリからの起動がうまくいかないものがあります。その場合は、以下の手順を試してください：
