...
そのため、コミット時に改行コードを`LF`に変更するので`autocrlf=input`と設定します。
...
ユーザーの設定は、\[user\]セクションで行います。ここで設定されたユーザー情報は、コミットログに表示されます。
...
また、下記の"`fixit`"というエイリアスは、コミットメッセージを編集せずに新たな修正を追加するコマンドです。このように、Git で行いたい操作をエイリアスを使うことで、1つのコマンドとして実装できます。
