...
atsushifx です。
この記事では、`WSL` (Windows Subsystem for Linux) を **GitHub から手動でダウンロードし、インストールする方法**を紹介します。
...
従来、Linux を利用するには、仮想マシンやデュアルブートの設定が必要でした。
...
これにより、`Linux` のバイナリを、ほぼそのまま `Windows` 上で実行できる環境が整います。
...
`WSL` には大きく分けて `WSL 1` と `WSL 2` の 2つのバージョンがあり、それぞれに特徴と利点があります。
用途に応じて選択することで、快適な開発環境を構築できます。
...
仮想化支援機構が無効な場合、`WSL 2`は起動せず、一部機能も動作しないという可能性があります。
...
表示が、`Enabled` であれば、仮想化支援機構は動作しています。
...
`WSL 2` を GitHub から手動で導入する場合、事前に`仮想化機能`などの必要な Windows 機能が有効になっているかを確認します。
...
WSL 2 を正しく動作させるには、いくつかの `Windows 機能`を有効にしておく必要があります。この章では、それらの機能の有効化手順と、正しく反映されたかの確認方法について解説します。
...
| Windows ハイパーバイザープラットフォーム | HypervisorPlatform | 軽量な仮想化APIで、`WSL 2` などの仮想技術を動かすエンジン的な役割 |
...
3. `.msi`のダウンロード:
   アセットから `wsl-<version>.<arch>.msi`となっているファイルをダウンロードする。
   ::: message
