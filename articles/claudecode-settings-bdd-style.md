...
コーディングにあたって課題が 1つありました。Claude に TDD/BDD を使ったコーディングをさせるのが意外と難しいという点です。
...
タスクを細かく分割しても、1回のテストが大きすぎる内容になってしまうのような、ユーザーにとって不都合なコーディングをすることがあります。
...
こちらの場合でも、`atsushifx式BDD`を指定すると、BDD でコーディングします。
...
Claude にコーディングを依頼するときに、「typescript-bdd-coder を使用して」と記述するとエージェントを使用してコーディングします。
...
れからは、Claude と一緒に、もっと気軽に、もっと柔軟にコードを書いていきましょう。
