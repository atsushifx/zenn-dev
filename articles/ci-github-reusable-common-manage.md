...
CI が失敗する原因として最も多いのが**認可の不足**。
とくに private リポジトリ参照時は挙動が揺らぎやすく、意図しない 403 に直結する。
問題を切り分けるには、権限の境界を正確に理解しなければならない。
...
- downstream workflows が途中で停止

原因:

- `permissions` が明示されていない
- `GITHUB_TOKEN` が **読み取り権限のみ**で不足
- PAT (`Personal Access Token`) の付与条件を満たしていない
...
- `persist-credentials: false`を徹底 (資格情報の残存防止)
...
Reusable Workflows を複数リポジトリで利用する場合、参照先の更新により CI/CD の動作が変化します。
安定運用のためには、バージョン管理を適切に行なう設計が欠かせません。

再現性を確保しつつ、破壊的変更の影響を抑えるために、Semantic Versioning を基盤としたタグ管理が有効です。

### 5.1 branch 固定・タグ固定・SHA 固定の比較

Reusable Workflows を参照する際の `<ref>` は、動作の安定性と運用負荷に直結します。
代表的な 3つの方法を比較し、選択基準を整理します。

<!-- markdownlint-disable line-length -->

| 方法         | 例                    | 特徴               | メリット                     | デメリット               | 推奨度 |
| ------------ | --------------------- | ------------------ | ---------------------------- | ------------------------ | ------ |
| ブランチ固定 | `@main` / `@releases` | 常に最新を参照     | 最新化が容易                 | 意図せぬ破壊的更新が発生 | △      |
| タグ固定     | `@v1.0.0` / `@r1.0.0` | バージョン固定     | 再現性が高い、影響制御が容易 | 更新は手動適用           | ◎      |
| SHA固定      | '@fd5eff...`          | 指定コミットで固定 | 最も安全 (改変対策)          | private参照で解決失敗    | ○      |
...
- private リポジトリ特有の事情
  SHA は private リポジトリに対して **ref 解決に失敗する既知の挙動**があります。
  そのため、private 運用では次の順で選択することが安全です。
...
Reusable Workflows の参照方法は、CI/CD 全体の安定性を左右する設計要素です。
「いつ壊れるかわからない運用」から脱するためにも、**タグ固定を基本方針とする**ことが最適解です。

### 5.2 semantic versioning と GitHub Actions の違い

Reusable Workflows にも Semantic Versioning を適用できます。
しかし、GitHub Actions 特有の挙動により、「似ているが一致しない」点があります。

#### Semantic Versioning の基本

一般的な SemVer の定義は次のとおりです。
...
| 種別  | 内容                   | CI/CDへの影響        |
| ----- | ---------------------- | -------------------- |
| MAJOR | 破壊的変更             | caller側で対策が必要 |
| MINOR | 後方互換ありの機能追加 | 更新推奨             |
| PATCH | バグ修正/内部改善      | 更新推奨             |

これにより、変更内容の意図を利用者が理解できます。

#### GitHub Actionsの仕様とのギャップ

GitHub Actions では、SemVer の慣習に近い形で ref を扱いますが、
以下の挙動が存在します。

1. バージョン範囲解決をしない
   npm のような`v1`→`v1.x.x`への自動変換を行ないません。

   ```yaml
   uses: org/.github/ci-common-lint.yml@v1
   ```

   これは SemVer ぽいが**固定ではなく、意図しない更新リスクがある**。
...
3. private repo では SHA 解決が安定しない
   特に Reusable Workflow の相対パス参照で問題化する

| 設定例    | 意味               | リスク                 |
| --------- | ------------------ | ---------------------- |
| `@main`   | 常に最新を取得     | 挙動に揺らぎ           |
| `@v1`     | メジャー互換の最新 | タグ上書きでも壊れる   |
| `@v1.2.3` | **完全固定**       | 要、手動更新。最も安全 |
| `@SHA246` | 完全固定           | private参照で失敗例    |

#### 実務に即した運用ポリシー

GitHub Actions における SemVer は、次の方針が最適解です。

- Semantic Versioning:

  ```bash
  MAJOR: breaking change 時に必ず上げる
  MINOR/PATCH: 上げても互換性維持

  Caller は常にタグ固定 (`vX.Y.Z`)
  ```

- caller workflow:
  下記のように、フルの SemVer を使用します。

  ```yaml
  uses: org/.github/.github/workflows/ci-common-lint.yml@r1.0.0
  ```
