...
APT は Debian 系ディストリビューションのパッケージ管理システムで、ソフトウェアの導入や更新、削除を効率的に管理できます。パッケージの依存関係も自動的に処理してくれる点が特徴です。
...
| `apt full-upgrade --dry-run` | 実行前にアップグレードの影響を確認 | パッケージの追加・削除予定を事前に確認可能 (例: `packages will be upgraded:`のようなメッセージが出力されます) |
