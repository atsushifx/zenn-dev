...
WSL (Windows Subsystem for Linux)[^1]にカスタマイズされた Debian[^2] をインポートして、開発環境をセットアップする方法を紹介します。
この方法を用いることで、一貫性のある開発環境を迅速に構築し、開発効率の向上が期待できます。
...
カスタマイズ済み Debian アーカイブは、開発効率を高める各種ツールとカスタマイズされた設定を含む`tar アーカイブ`[^3]です。
このアーカイブには、`Git`などの開発ツールや`Vim`の設定などが含まれています。
この記事で使うアーカイブは、[環境構築の記事まとめ](https://zenn.dev/atsushifx/articles/wsl2-debian-setup-matome) で事前にセットアップしたされたものです。

個人の認証情報を削除するため、`dotfiles`は直接ダウンロードしたものを使用しています。
...
インポートした`tarアーカイブ`は、WSL上の指定したディストリビューションとして実行できます。
また、`tarアーカイブ`時点での設定を再現できるため、環境設定の時間を大幅に短縮できます。

### 2.1. wsl インポートのコマンドライン

`wsl --import`[^4] コマンドは、`tarアーカイブ`形式の Linux ディストリビューションを WSL上にインポートして、WSL上で実行可能な状態にします。
...
カスタマイズ済みの Debian アーカイブをダウンロードし、WSL にインポートする手順を詳細に説明します。
これにより、セットアップの過程が明確になり、ユーザーが迷わずに作業を進められます。
...
ここでは、UNIX/Linux のディレクトリ規格である`XDG Base Directory`[^6]にしたがっています。
...
デフォルトユーザーアカウントを、開発者のアカウントに変更する方法を説明します。
開発者自身のアカウントに変更することで、開発者が環境をカスタマイズできます。

### 4.1 ユーザーアカウントの変更

ユーザーアカウントを`pwruser`から自分のアカウントに変更する必要があります。
変更には、アカウントを新しいアカウントに変更するスクリプト`move_useraccount.sh`を使用します。
...
以上の手順で、WSL の設定は完了です。
完了後、WSL を再起動して、新しい設定を反映させます。
これにより、カスタマイズされた Debian 環境が正しく設定され、使用可能になります。
...
Debian の環境設定は、`dotfiles`[^7]と呼ばれる設定ファイルをまとめたリポジトリで管理しています。
インポートした Debian では、`dotfiles`はバージョン管理されていません。
既存の`dotfiles`を削除し、フォークした`dotfiles`を組み込むことで`dotfiles`をバージョン管理します。

このとき、インストール済みの`what`コマンドも削除されますので、あらためて、`what`コマンドを再インストールします。
