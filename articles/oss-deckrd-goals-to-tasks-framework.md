...
`deckrd` は、Agent Skills を用いて設計プロセスを支援するフレームワークです。
`deckrd` は「Your Goals to Tasks」という考え方に基づき、要求を実装可能なタスクリストにまで落とし込むことを目的としています。

もともとは `SDD`（仕様書駆動開発）における GitHub Issue をコーディングするエージェントコマンドとして構想されたものです。
試行錯誤を重ねるなかで、単なるタスク生成ではなく、**設計の思考過程そのものをドキュメントとして残すフレームワーク**へと発展しました。

`deckrd` は、軽量で試行錯誤を前提としたアジャイルな開発スタイルに適したドキュメントフレームワークです。
気軽に試して、プログラミングを愉しんでください。
Enjoy!
...
この章では、AI に設計を任せたときに起きやすい問題を整理し、なぜ「結果だけが残る設計」になりやすいのかを明らかにします。

### 1.1 AI に設計を任せると、なにが失われるのか
...
*図 1-1: AI に設計を任せたときに起きやすい断片化の構造*
...
### 1.2 AIとの対話では、設計判断が記録に残らない (記録されない 問題)

ここでの問題は、対話が保存されないことにあります。
多くの AI 活用は、チャットベースで行われます。
この形式では、やり取りそのものはログとして残りますが、設計判断を整理した記録として保存されることはありません。

対話のなかでは、前提条件や判断理由、別案の検討といった情報が頻繁に現れます。
しかし、それらは発言の流れのなかで消費されやすく、
後から参照できる形には整理されません。

その結果、次のような情報が設計の記録として残らなくなります。

- 判断に用いた前提条件
- 結論に至った理由
- 採用しなかった案とその理由

チャットログは存在していても、どの判断が、どの前提にもとづいて行われたのかを説明する資料としては扱いづらくなります。

### 1.3 設計の文脈が再現できない (再現できない 問題)

次に問題になるのは、判断の経路を再現できない点です。
生成 AI は、与えられたプロンプトに対して整合性のある出力を返します。
しかし、その整合性は対話のその場で成立するものであり、過去の判断や前提が、設計として再現可能な形で保持されるわけではありません。

対話を重ねるうちに、どの前提にもとづいて判断したのか、どの段階で結論が切り替わったのかが曖昧になっていきます。
結果として、同じ目的を与えても、同じ設計にたどり着けなくなります。

問題は、結論が変わることではありません。判断に至るまでの文脈を再現できないために、
設計を説明できず、再利用もできなくなる点にあります。

### 1.4 結果は残っても、設計としては使えない

AI によって生成された仕様やタスクは、一見すると整理されているように見えます。
しかし、そこに残っているのは結論だけであり、なぜその仕様やタスクに至ったのかという設計上の判断は含まれていません。

この状態では、次のような情報が設計として欠落します。

- 判断に至った理由
- 要求との対応関係
- 前提条件の範囲

その結果、後から仕様を見直したり変更したりする際に、どこから設計を組み直せばよいのかがわからなくなります。

これは特定のツールや手法に限った問題ではありません。
SDD などの AI 支援開発においても、設計判断が記録されないままタスクや Issue だけが積み上がることで、
全体像を説明できない状態に陥りがちです。

タスクは増えていく一方で、それらがどの要求にもとづき、どの判断によって導かれたのかを辿れない。
この断片化が、AI に設計を任せたときに起きやすい最終的な帰結です。

### 1.5 問題は「AI」ではなく「設計の扱い方」にある

ここまで見てきた問題は、AI の性能や精度に起因するものではありません。
AI による出力が不十分なのではなく、その出力や対話の過程を、設計としてどのように扱っているかに原因があります。

AI との対話では、前提条件や判断理由、検討の経緯といった情報が頻繁に現れます。
しかし、それらが設計の記録として整理されず、対話の流れのなかで消費されてしまうと、設計は結果だけが残る形になります。

文脈が固定されないまま対話を重ねることで、判断の前提は揺れ、結論はばらつき、
仕様やタスクはあっても、それを支える設計意図を説明できなくなります。

要求・前提・判断・結論が分離されたまま使われる限り、AI を用いた設計は、何度でも断片化します。
...
`deckrd` は、この「Your Goals to Tasks」という発想を中核に据え、設計の思考過程をドキュメントとして残すことを目的としています。
...
Goal を具体化し共有できても、それだけで実装に至るわけではありません。
設計を進める過程では、作業内容に至るまでの判断を整理し、記録します。
...
この流れは、作業を工程として分割することを目的としたものではありません。
各段階で「どのように判断するのか」を切り分け、思考の役割を整理するための枠組みです。
...
`deckrd` における Implementation は、コードを書く工程そのものではありません。
...
![Decision Records connect Task back to Requirements and Goals](/images/articles/oss-deckrd/deckrd-dr.png)

*図2-1: Decision Records(DR)が、各フェーズを横断的に結びTasksからGoalsまでを逆にたどれる構造*

判断を DR として残しておくことで、Tasks から Requirements、さらに Goal までの関係を、設計の経緯を含めて説明しやすくなります。
その結果、設計内容を振り返ったり見直したりする際の手がかりが得られます。
...
`deckrd` は Tasks を生成しますが、タスクを作ること自体を目的とした道具ではありません。
...
その結果、設計全体の背景や意図を保ったまま、見直しや再検討が可能になります。
