---
title: "WSL 2: Debianを日本語環境に設定する"
emoji: "🐧"
type: "tech"
topics: ["WSL", "Debian", "apt", "環境構築", "日本語" ]
published: false
---
...
英語に慣れているユーザーであれば問題ありませんが、Linux を使いはじめたばかりの初心者にとっては、英語の出力が作業の妨げになることがあります。
...
- コマンドの結果やエラー内容が直感的に理解しにくい
- アプリケーションによっては、日本語表示が崩れる
- 日本語入力や表示に関わる設定が適切に機能しない
...
2. Debian の終了:
   Debian が終了し、もとの`powershell`にもどります。
   ![Windows Terminal](/images/articles/wsl2-debian/ss-terminal-normal.png)
...
この設定により、日本語ロケールが有効化され、再起動後にシステム出力が日本語になります。
...
タイムゾーンを CLI から競っているする場合は、以下の手順で行ないます。
...
WSL の設定は、ターミナルの再起動だけでは反映されないことがあります。以下の手順で、WSL を完全に再起動します。

#### PowerShell での WSL 再起動
